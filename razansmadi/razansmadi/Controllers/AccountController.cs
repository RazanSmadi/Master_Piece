...
namespace razansmadi.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            string id = db.AspNetUsers.Where(x => x.Email == model.Email).Select(x => x.Id).FirstOrDefault();
            var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe, shouldLockout: false);
            string role = db.AspNetUserRoles.Where(x => x.AspNetUser.Email == model.Email).Select(y => y.RoleId).FirstOrDefault();
            switch (result)
            {
...
return View(model);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, string FirstName, string LastName, int Phone, HttpPostedFileBase Customer_Img)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<ActionResult> RegisterOwner(RegisterViewModel model, string FirstName, string LastName, int Phone, HttpPostedFileBase Customer_Img)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
